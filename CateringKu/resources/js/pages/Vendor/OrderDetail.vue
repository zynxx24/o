...
function updateStatus(status: string) {
    router.patch(`/vendor-panel/orders/${props.order.order_id}/status`, { status }, { preserveScroll: true })
}
...
<template>
    <Head :title="`Pesanan ${order.order_number} - Vendor CateringKu`" />
    <VendorLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <Link href="/vendor-panel/orders" class="text-gray-400 dark:text-gray-500 hover:text-ck-primary transition-colors">
            ...
            <div class="bg-white dark:bg-[#1f2037] rounded-2xl border border-gray-100 dark:border-[#2a2c45] p-6 shadow-sm">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <h2 class="text-xl font-bold text-gray-800 dark:text-gray-100 mt-2">{{ order.user?.name }}</h2>
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ order.user?.email }} · {{ order.user?.phone }}</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <span :class="statusColors[order.status]" class="px-3 py-1.5 rounded-lg text-xs font-bold">{{ statusLabels[order.status] }}</span>
                        <select v-if="order.status !== 'cancelled' && order.status !== 'completed'" @change="(e: any) => updateStatus(e.target.value)" :value="order.status" class="text-xs border border-gray-200 dark:border-[#2a2c45] rounded-lg px-3 py-2 bg-white dark:bg-[#161729] text-gray-700 dark:text-gray-300">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white dark:bg-[#1f2037] rounded-2xl border border-gray-100 dark:border-[#2a2c45] p-6 shadow-sm">
                        ...
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div><span class="text-gray-400 dark:text-gray-500 text-xs">Jumlah Tamu</span><p class="font-medium text-gray-800 dark:text-gray-200">{{ order.num_people }} orang</p></div>
                            <div class="col-span-2"><span class="text-gray-400 dark:text-gray-500 text-xs">Alamat Pengiriman</span><p class="font-medium text-gray-800 dark:text-gray-200">{{ order.delivery_address }}{{ order.delivery_city ? ', ' + order.delivery_city : '' }}</p></div>
                            <div v-if="order.special_request" class="col-span-2"><span class="text-gray-400 dark:text-gray-500 text-xs">Catatan Khusus</span><p class="font-medium text-gray-800 dark:text-gray-200">{{ order.special_request }}</p></div>
                        </div>
                    </div>
                                ...
                                <tbody class="divide-y divide-gray-50 dark:divide-[#2a2c45]">
                                    <tr v-for="item in order.items" :key="item.order_item_id">
                                        <td class="py-3">
                                            ...
                                            <p v-if="item.notes" class="text-xs text-gray-400 dark:text-gray-500">{{ item.notes }}</p>
                                        </td>
