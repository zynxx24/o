...
<template>
    ...
    >
        <div class="relative w-5 h-5 overflow-hidden">
            ...
            >
                <svg v-if="resolvedAppearance === 'light'" class="w-5 h-5 absolute inset-0 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <Transition
                ...
                leave-to-class="opacity-0 rotate-90 scale-0"
            >
                <svg v-if="resolvedAppearance === 'dark'" class="w-5 h-5 absolute inset-0 text-indigo-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                </svg>
            </Transition>
        </div>
        <span class="hidden sm:inline text-xs">{{ resolvedAppearance === 'dark' ? 'Gelap' : 'Terang' }}</span>
    </button>
    ...
    >
        <svg v-if="resolvedAppearance === 'light'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
        </svg>
    ...
    <button
        ...
        :class="resolvedAppearance === 'dark' ? 'bg-indigo-600' : 'bg-gray-300'"
        :title="resolvedAppearance === 'dark' ? 'Mode terang' : 'Mode gelap'"
    >
        <span
            class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-white shadow-sm transition-transform duration-300"
            :class="resolvedAppearance === 'dark' ? 'translate-x-7' : 'translate-x-1'"
        >
            <svg v-if="resolvedAppearance === 'light'" class="w-3 h-3 text-amber-500" fill="currentColor" viewBox="0 0 20 20">
